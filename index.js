...
const routes = require('./Routes/studentRoute'); // Check the correct path based on your project structure
...
app.get('/addOpp', (req, res) => {
    res.render('addOpp');
});

app.get('/myOpp', (req, res) => {
    res.render('myOpp');
});
...
app.post('/student/signup', async (req, res) => {
    ...
    try {
        ...
        User.findUserByUsername(username, (err, existingUser) => {
            if (err) {
                return res.render('error', { message: 'Error checking username availability' });
            }

            if (existingUser) {
                // Username already exists, prompt the user to choose a different one
                studentErrors.usernameExists = true;
                return res.render('signup', { studentErrorMessage: 'Username is already taken. Choose a different one.', studentErrors });
            }
            ...
            User.createUser(username, password, 'student', firstName, lastName, studentId, email, department, (err, newUser) => {
                if (err) {
...
app.post('/admin/signup', (req, res) => {
    ...
    User.findUserByUsername(username, (err, existingUser) => {
        ...
        if (existingUser) {
            ...
            return res.render('signup', { adminErrorMessage: 'Username is already taken. Choose a different one.', adminErrors });
        }
        ...
        // If username is available, create the new user
        User.createUser(username, password, 'admin', firstName, lastName, null, email, null, (err, newUser) => {
            if (err) {
                return res.render('error', { message: 'Error signing up' });
            }
            res.redirect('/login');
...
app.get('/student_dashboard', (req, res) => {
    ...
    res.render('student_dashboard', { userData, opportunities: opportunitiesData });
});
